...
@section('content')
  ...
  @endif
  <div class="list-statuts">
    <div class="content pt-6 w-[95%] mx-auto my-5 flex flex-col gap-11">
      <div class="lis-statuts w-full bg-gray-100 rounded-lg border border-gray-200 p-5 flex flex-col gap-6">
        <h1 class="text-2xl font-medium ">List Statuts</h1>
        <div>
          <button class="bg-blue-600 py-2 px-4 rounded-md text-white hover:shadow-md" id="openCreateStatusModalBtn">Add
            Statut</button>
        </div>
            ...
            @foreach ($statuts as $statut)
              <tr>
                ...
                <td
                  ...
                  <a href="{{ route('edit-statut', ['id'=>$statut->id]) }}" class="text-white text-base font-medium bg-[#4DA845] rounded-lg py-1 w-1/2">Edit</a>
                  <form action="{{ route('delete-statut', ['id'=>$statut->id]) }}" class="w-1/2" method="POST">
                    @csrf
                    @method('DELETE')
  ...
  {{-- Create Statut --}}
  <div id="createStatusModal" class="modal hidden fixed inset-0 z-50 overflow-hidden bg-gray-500 bg-opacity-50">
    <div class="modal-content relative bg-white shadow-md mx-auto my-20 w-96 p-6 rounded-lg">
      ...
      <h2 class="text-xl font-bold mb-4">Créer un nouveau statut</h2>
      <form id="createStatusForm" action="{{ route('store-statut') }}" method="POST">
        @csrf
        <div class="mb-4">
