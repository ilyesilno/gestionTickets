      ...
      <form id="creerTicketsForm" action="{{ route('store-ticket') }}" method="POST">
        ...
        <div class="mb-3">
          ...
          <textarea name="description" id="description" rows="3"
            class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 resize-vertical">{{ old('description') }}</textarea>
          @error('description')
            <span class="text-red-600 text-sm">{{ $message }}</span>
          @enderror
        </div>
        <div class="mb-3">
          ...
          <select name="priorite" id="priorite"
            ...
            <option value="Moyen" {{ old('priorite') == 'Moyen' ? 'selected' : '' }}>Moyen</option>
            <option value="Haut" {{ old('priorite') == 'Haut' ? 'selected' : '' }}>Haut</option>
          </select>
          @error('priorite')
            <span class="text-red-600 text-sm">{{ $message }}</span>
          @enderror
        ...
        <div class="mb-3">
          ...
          <select name="categorie" id="categorie"
            ...
            <option value="Bug" {{ old('categorie') == 'Bug' ? 'selected' : '' }}>Bug</option>
